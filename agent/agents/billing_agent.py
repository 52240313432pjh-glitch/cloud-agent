...
class UserIdInjector(ToolCallInterceptor):
    ...
    async def __call__(
        ...
        if user_id:
            ...
            trace_log(trace_id, "tool_call_start", user_id=user_id, tool=request.name, args=new_args)
            try:
                ...
                trace_log(
                    ...
                    user_id=user_id,
                    tool=request.name,
                    latency_ms=trace_ms(tool_start),
        ...
        tool_start = time.perf_counter()
        trace_log(trace_id, "tool_call_start", tool=request.name, args=request.args)
        try:
...
"""
        ...
        if self.mcp_manager is not None:
            tools = await self.mcp_manager.get_tools_by_names(allowed_tool_names)
        else:
